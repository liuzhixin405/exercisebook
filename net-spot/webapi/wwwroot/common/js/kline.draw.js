...
String.fromFloat1 = function (a, c) {
	var n = Number(a);
	if (n < 1000) return String.fromFloat(a, c);
	if (n < 1000000) {
		var d = n / 1000;
		return Number(String.fromFloat(d, c)) + 'K';
	} else {
		var d = n / 1000000;
		return Number(String.fromFloat(d, c)) + 'M';
	}
};
...
//    var a = GLOBAL_VAR.market_from_name; // 得到a变量
//    a += " ";
...
Chart.prototype.updateDataAndDisplay = function () {
    ...
    this.setTitle();
    ChartManager.getInstance().setCurrentDataSource("frame0.k0", "BTC-Q." + this._market_from + "." + this._time);
...
TableLayout.prototype.measure = function (a, u, m) {
    ...
    var data = I.getDataSource(this.getDataSourceName());
    var digits = data.getDecimalDigits();
    var vDigits = data.getVolumeDigits();
    if (e.getFirstIndex() >= 0) {
        ...
        for (C = 0, j = 0, f = z; C < this._areas.length && j < l.length; C += 2) {
            ...
            for (var B in q) {
                ...
                while (j < l.length) {
                    var E = F.getRequiredWidth(a, s[j].min, dt, (A.getName() != "frame0.k0.main"));
                    var D = F.getRequiredWidth(a, s[j].max, dt, (A.getName() != "frame0.k0.main"));
                    if (Math.max(E, D) < f) {
                        break
                    }
                    j++;
...
ChartManager.prototype.init = function () {
    ...
    DefaultTemplate.loadTemplate("frame0.k0", "BTC-Q");
...
ChartManager.prototype.setNormalMode = function () {
    ...
    $("#chart_toolpanel .chart_toolpanel_button").removeClass("selectsed");
    $("#chart_CrossCursor").parent().addClass("selectsed");
    if (this._drawingTool == ChartManager.DrawingTool.Cursor) {
        this.showCursor();
        $("#mode a").removeClass("selected");
        $("#chart_toolpanel .chart_toolpanel_button").removeClass("selectsed");
        $("#chart_Cursor").parent().addClass("selectsed")
    } else {
        this.hideCursor()
    }
};
...
ChartManager.prototype.hideCursor = function (a) {
	if (a === undefined) {
        a = "default"
    }
    this._mainCanvas.style.cursor = a;
...
ChartManager.prototype.setIndicator = function (h, a) {
    ...
    g.setIndicator(f);
    this.setRange(h, b);
    b.setPaddingTop(20 * kScale);
    b.setPaddingBottom(4 * kScale);
    b.setMinInterval(20 * kScale);
...
Timeline._ItemWidth = [1 * kScale, 3 * kScale, 3 * kScale, 5 * kScale, 5 * kScale, 7 * kScale, 9 * kScale, 11 * kScale, 13 * kScale, 15 * kScale, 17 * kScale, 19 * kScale, 21 * kScale, 23 * kScale, 25 * kScale, 27 * kScale, 29 * kScale];
Timeline._SpaceWidth = [1 * kScale, 1 * kScale, 1 * kScale, 1 * kScale, 1 * kScale, 1 * kScale, 1 * kScale, 1 * kScale, 1 * kScale, 1 * kScale, 2 * kScale, 2 * kScale, 2 * kScale, 2 * kScale, 3 * kScale, 3 * kScale, 3 * kScale];
Timeline.PADDING_LEFT = 4 * kScale;
Timeline.PADDING_RIGHT = 8 * kScale;
...
Timeline.prototype.getVerticalGridLines = function () {
    return this._verticalGridLines
};
Timeline.prototype.setVerticalGridLines = function (a) {
    this._verticalGridLines = a
};
...
Timeline.prototype.update = function () {
    ...
    if (d == this.getLastIndex() && this._maxIndex > this.getLastIndex()) {
    	b.setUpdateMode(DataSource.UpdateMode.Refresh);
    }
...
Range.prototype.updateGradations = function () {
    ...
    do {
        this._gradations.push(a.toFixed(2));
        a -= b
...
MainDataSource.prototype.getVolumeDigits = function () {
    return this._volumeDigits
};
...
MainDataSource.prototype.getDataAt = function (a) {
	if (a >= this._dataItems.length || a < 0) {
		return this._dataItems[this._dataItems.length - 1];
	}
...
MainDataSource.prototype.update = function (c) {
    ...
    if (c.length > 0) {
	    ...
	    for (f = 0; f < b; f++) {
	        j = c[f];
	        for(var i in j) {
	        	if (i == 'volume') continue;
	        	k = this.calcDecimalDigits(j[i]);
	        	if (this._decimalDigits < k) {
	                this._decimalDigits = k
	            }
			}
	        
	        this._volumeDigits = 2;
...
//	        }
    ...
    if (this._dataItems.length == 0) {
    	...
    	if (data.length > 2) {
    		if (data.length > this._dataItems.length) {
    			this.setUpdateMode(DataSource.UpdateMode.Refresh);
		    	this._dataItems = data;
    		} else {
	    		var i = data.length - 1;
	    		var oldObj = this._dataItems[this._dataItems.length - 1];
				var newObj = data[i];
				var _i = 0;
				for(var k in oldObj) {
    		...
    		for (var i = 0; i < data.length; i++) {
				...
				if(_i) {
					if (oldObj.date == newObj.date) {
						this._dataItems[this._dataItems.length - 1] = newObj;
						this._updatedCount++;
					} else if (oldObj.date < newObj.date) {
						this._dataItems.push(newObj);
						this._appendedCount++;
					}
				}
    		}
...
IndicatorDataProvider.prototype.updateData = function () {
    ...
    var tline = e.getTimeline(this.getDataSourceName());
    if (d.getDataCount() < 1) {
        return
    }
...
Theme.Color = {
    ...
    White: theme_color_id++,
    Black: theme_color_id++,
...
DarkTheme.prototype.__construct = function () {
    this._colors = [];
    this._colors[Theme.Color.Positive] = "#47BE87";		// 涨价 绿色
    this._colors[Theme.Color.Negative] = "#E16363";		// 跌价 红色
    this._colors[Theme.Color.PositiveDark] = "#004718";	// 深度买
    this._colors[Theme.Color.NegativeDark] = "#3b0e08";	// 深度卖
    this._colors[Theme.Color.Unchanged] = "#fff";
    this._colors[Theme.Color.White] = "#fff";
    this._colors[Theme.Color.Black] = "#000";
    this._colors[Theme.Color.Background] = "#0d1923";	// 背景色
    this._colors[Theme.Color.Cursor] = "#4a5f78";		// 十字光标
    this._colors[Theme.Color.RangeMark] = "#f9ee30";	// 最新价
    this._colors[Theme.Color.Indicator0] = "#ddd";		// 指标线
    this._colors[Theme.Color.Indicator1] = "#f9ee30";	// 指标线
    this._colors[Theme.Color.Indicator2] = "#f600ff";	// 指标线
    this._colors[Theme.Color.Indicator3] = "#6bf";		// 指标线
    this._colors[Theme.Color.Indicator4] = "#a5cf81";	// 指标线
    this._colors[Theme.Color.Indicator5] = "#e18b89";	// 指标线
    this._colors[Theme.Color.Grid0] = "#1b2733";		// 网格线
    this._colors[Theme.Color.Grid1] = "#4a5f78";		// 坐标线
    ...
    this._colors[Theme.Color.Text2] = "#94ABC0";			// 坐标刻度文字颜色
    this._colors[Theme.Color.Text3] = "#fff";				// 十字光标文字颜色
    this._colors[Theme.Color.Text4] = "#94abc0";			// 高开低收文字
    this._colors[Theme.Color.Text5] = "#585858";			// 十字光标文字底色
    this._colors[Theme.Color.LineColorNormal] = "#94ABC0";
    ...
    this._fonts = [];
    this._fonts[Theme.Font.Default] = (12 * kScale) + "px Microsoft YaHei";	// 字体
};
var LightTheme = create_class(Theme);
//白色背景
LightTheme.prototype.__construct = function () {
    this._colors = [];
    this._colors[Theme.Color.Positive] = "#62C378";		//涨价 绿色
    this._colors[Theme.Color.Negative] = "#D55959";		//跌价 红色
    ...
    this._colors[Theme.Color.Cursor] = "#4A5F78";		// 十字光标
    this._colors[Theme.Color.RangeMark] = "#f9ee30";	// 最新价
    this._colors[Theme.Color.Indicator0] = "#2fd2b2";	// 指标线
    this._colors[Theme.Color.Indicator1] = "#ffb400";	// 指标线
    this._colors[Theme.Color.Indicator2] = "#e849b9";	// 指标线
    this._colors[Theme.Color.Indicator3] = "#1478c8";	// 指标线
    this._colors[Theme.Color.Indicator4] = "#a5cf81";	// 指标线
    this._colors[Theme.Color.Indicator5] = "#e18b89";	// 指标线
    this._colors[Theme.Color.Grid0] = "#f4f6f9";		// 网格线
    this._colors[Theme.Color.Grid1] = "#94ABC0";		// 坐标线
    ...
    this._colors[Theme.Color.Text2] = "#4a5f78";			// 坐标刻度文字颜色
    this._colors[Theme.Color.Text3] = "#fff";				// 十字光标文字颜色
    this._colors[Theme.Color.Text4] = "#4a5f78";			// 高开低收文字
    this._colors[Theme.Color.Text5] = "#585858";			// 十字光标文字底色
    ...
    this._fonts = [];
    this._fonts[Theme.Font.Default] = (12 * kScale) + "px Tahoma"
};
...
TemplateMeasuringHandler.onMeasuring = function (c, b) {
    ...
    if (e == "timeline") {
        c.setMeasuredDimension(d, 22 * kScale)
    }
};
...
Template.createMainChartComps = function (c) {
    ...
    f.setPaddingTop(38 * kScale);
    f.setPaddingBottom(10 * kScale);
...
Plotter.drawDashedLine = function (b, c, l, a, k, g, j, color) {
    if (g < 2) {
        g = 2
    }
    var e = a - c;
...
CGridPlotter.prototype.Draw = function (c) {
    ...
    var lines = p.getVerticalGridLines();
    if (lines.length > 0) {
    	for (var d in lines) {
        	Plotter.drawLine(c, lines[d].x, b.getTop(), lines[d].w, b.getBottom());
        }
    }
    
    var l = j.getGradations();
    for (var d in l) {
    	Plotter.drawLine(c, b.getLeft(), j.toY(l[d]), b.getRight(), j.toY(l[d]));
    }
	
    if (this.getAreaName() == 'frame0.k0.main') {
		var img= document.getElementById("img");
		c.globalAlpha = 0.65;
		c.drawImage(img, 10 * kScale, b.getHeight() - 26 * kScale - 10 * kScale);
		c.globalAlpha = 1;
    }
...
MainInfoPlotter.prototype.Draw = function (c) {
    ...
    var a = {
        x: w.getLeft() + 4 * kScale,
        y: w.getTop() + 4 * kScale,
        w: w.getWidth() - 8 * kScale,
        h: 20 * kScale
    };
    var aa = {
        x: w.getLeft() + 4 * kScale,
        y: w.getTop() + 20 * kScale,
        w: w.getWidth() - 8 * kScale,
        h: 20 * kScale
    };
    var s = e.getSelectedIndex();
    if (s < 0 || s > e.getMaxIndex()) {
    	s = e.getLastIndex() - 1;
    }
    if (s < 0) return;
    ...
    var D = B.getLanguage();
    var df = y.getColor(Theme.Color.Text4);
    var updown = y.getColor(Theme.Color.Positive);
    if (C.close < C.open) {
    	updown = y.getColor(Theme.Color.Negative);
    }
    
    // 高开低收
    if (!Plotter.drawString(c, MSG1['time'] + k + "-" + A + "-" + z + "  " + f + ":" + l, a)) {
        return
    }
    if (!Plotter.drawString(c, MSG1['open'], a)) {
        return
    }
    c.fillStyle = updown;
    if (!Plotter.drawString(c, C.open.toFixed(i), a)) {
        return
    }
    c.fillStyle = df;
    if (!Plotter.drawString(c, MSG1['high'], a)) {
        return
    }
    c.fillStyle = updown;
    if (!Plotter.drawString(c,C.high.toFixed(i), a)) {
        return
    }
    c.fillStyle = df;
    if (!Plotter.drawString(c, MSG1['low'], a)) {
        return
    }
    c.fillStyle = updown;
    if (!Plotter.drawString(c, C.low.toFixed(i), a)) {
        return
    }
    c.fillStyle = df;
    if (!Plotter.drawString(c, MSG1['close'], a)) {
        return
    }
    c.fillStyle = updown;
    if (!Plotter.drawString(c, C.close.toFixed(i), a)) {
        return
    }
    c.fillStyle = df;

    // 涨幅
    var dd = (C.close - C.open) / C.open * 100;
    if (!Plotter.drawString(c, MSG1['change'], a)) {
       //  return
    }
    c.fillStyle = updown;
    if (!Plotter.drawString(c, dd.toFixed(2) + " %", a)) {
       //  return
    }
    ...
    if (!Plotter.drawString(c, MSG1['amplitude'] + d.toFixed(2) + " %", a)) {
        // return
    }
    ...
    if (b.getName() == "SAR") {
    	list = b.getName() + "(" + b.getOutputAt(0)._exprA._exprA._value + ","
    		+ b.getOutputAt(0)._exprA._exprB._value + ","
    		+ b.getOutputAt(0)._exprA._exprC._value + ","
    		+ b.getOutputAt(0)._exprA._exprD._value;
    } else if (b.getName() == "BOLL") {
    	list = b.getName() + "(" + b.getParameterAt(0).getValue();
    } else {
	    for (p = 0; p < t; p++) {
	        ...
	        if (list == "") {
	        	list = b.getName() + "(" + b.getParameterAt(p).getValue();
	        } else {
	        	list += "," + b.getParameterAt(p).getValue();
	        }
	    }
    }
    if (list != "") {
    	list += ")";
    	if (!Plotter.drawString(c, list, aa)) {
            return
        }
    }
...
IndicatorInfoPlotter.prototype.Draw = function (c) {
    ...
    var l = {
        x: b.getLeft() + 4 * kScale,
        y: b.getTop() + 4 * kScale,
        w: b.getWidth() - 8 * kScale,
        h: 20 * kScale
    };
    ...
    for (e = 0; e < d; e++) {
        ...
        f = "  " + i.getName() + ": " + String.fromFloat1(o, 2); //o.toFixed(2);
        g = i.getColor();
        if (g === undefined) {
...
MinMaxPlotter.prototype.drawMark = function (d, k, e, j, g, a, h, l) {
    ...
    Plotter.drawLine(d, f, j, b, j + 2 * kScale);
    Plotter.drawLine(d, f, j, b, j - 2 * kScale);
...
TimelinePlotter.prototype.Draw = function (b) {
    ...
    for (; u < w; u++) {
        if (TimelinePlotter.TIME_INTERVAL[u] % t == 0) {
            if ((TimelinePlotter.TIME_INTERVAL[u] / t) * e.getColumnWidth() > 60 * kScale) {
                break
            }
        }
    }
    ...
    var maxIndex = e.getMaxItemCount();
    var A = new Date();
    ...
    var q = z.getTop();
    var H = z.getMiddle();
    var over = maxIndex - (k - h);
    var last = s.getDataAt(k - 1).date;
    
    var oneYearWidth = (e.getItemWidth() + e.getSpaceWidth()) * 12 / kScale;
    var monthGap = 12;
    if (oneYearWidth < 80) {
        var years = parseInt(160 / oneYearWidth); 
        monthGap *= years;
    } else {
        monthGap = parseInt(oneYearWidth / 80);
    }
    
    var gotYear = 0;
    for (var y = h; y < k + over; y++) {
        var F = last;
        var itval = s.getDataAt(y).interval;
        if (y < k) {
        	F = s.getDataAt(y).date;
        } else {
        	if (itval == "1M") {
        ...
        if (itval == "1M") {
        	if (monthGap >= 12) {
        		var yearGap = parseInt(monthGap / 12);
        		if (E == 1) {
        			if (gotYear == 0) {
        				p = o.toString() + "年";
        				gotYear = o;
...
RangePlotter.prototype.getRequiredWidth = function (b, a, digits, kmformat) {
    var c = ChartManager.getInstance();
    var d = c.getTheme(this.getFrameName());
    b.font = d.getFont(Theme.Font.Default);
    if (kmformat) {
    	return b.measureText(String.fromFloat1(a, digits)).width + 14 * kScale;
    } else {
    	return b.measureText((Math.floor(a)).toFixed(digits)).width + 14 * kScale;
    }
};
// 刻度
RangePlotter.prototype.Draw = function (c) {
    ...
    var digits = 2;
    var vDigits = 2;
    var data = p.getDataSource(this.getDataSourceName());
    if (data.getDataCount() > 0) {
    	digits = data.getDecimalDigits();
    	vDigits = data.getVolumeDigits();
    }
    ...
    for (var d in l) {
        ...
        if (j) {
        	c.fillText(String.fromFloat(l[d], digits), e + 11 * kScale, k);
        } else {
        	c.fillText(String.fromFloat1(l[d], digits), e + 11 * kScale, k);
        }
    }
    if (f.length > 0) {
...
LastVolumePlotter.prototype.Draw = function (b) {
    ...
    b.textAlign = "left";
    b.textBaseline = "middle";
    b.fillStyle = e.getColor(Theme.Color.Positive);
    b.strokeStyle = e.getColor(Theme.Color.Positive);
...
LastClosePlotter.prototype.Draw = function (b) {
    ...
    var j1 = c._dataItems[c._dataItems.length - 1].open;
    if (j1 <= g.getMinValue() || j1 >= g.getMaxValue()) {
        j1 = j;
    }
    var textColor = Theme.Color.Black;
    var updown = Theme.Color.Positive;
    if (j1 > j) {
    	updown = Theme.Color.Negative;
    ...
    var d = k.getTheme(this.getFrameName());
    b.fillStyle = d.getColor(updown);
    b.fill();
    b.strokeStyle = d.getColor(updown);
    b.stroke();
    b.font = d.getFont(Theme.Font.Default);
    b.textAlign = "left";
    b.textBaseline = "middle";
    b.fillStyle = d.getColor(textColor);
    b.fillText(String.fromFloat(j, c.getDecimalDigits()), a.getLeft() + 11 * kScale, h + 2 * kScale);
    
    b.fillStyle = d.getColor(updown);
    Plotter.drawDashedLine(b, 0, h, a.getLeft(), h, 3 * kScale, 2 * kScale);
};
...
TimelineSelectionPlotter.prototype.Draw = function (b) {
    ...
    b.fillStyle = m.getColor(Theme.Color.Text5);
    b.fillRect(g - 52.5 * kScale, l.getTop() + 2.5 * kScale, 106 * kScale, 16 * kScale);
    b.strokeStyle = m.getColor(Theme.Color.Text5);
    b.strokeRect(g - 52.5 * kScale, l.getTop() + 2.5 * kScale, 106 * kScale, 16 * kScale);
    b.font = m.getFont(Theme.Font.Default);
    b.textAlign = "center";
    b.textBaseline = "middle";
    b.fillStyle = m.getColor(Theme.Color.Text3);
    
    var dataAt = j.getDataAt(c.getSelectedIndex());
    if (dataAt == null) return;
    
    var itval = dataAt.interval;
    var date = dataAt.date;
    if (j.getDataCount() >= 2 && c.getLastIndex() <= c.getSelectedIndex()) {
    	if (itval == "1M") {
    		var lastDate = new Date(date);
    		lastDate.setMonth(lastDate.getMonth() + c.getSelectedIndex() - c.getLastIndex() + 1);
			lastDate.setDate(1);
			date = lastDate.getTime();
    	} else {
    		var t = j.getDataAt(1).date - j.getDataAt(0).date;
    		date += t * (c.getSelectedIndex() - c.getLastIndex() + 1);
    	}
    }
    ...
    var h = "";
	if (itval == "1M" || itval == "1D" || itval == "3D" || itval == "1W") {
		h = year + "-" + a + "-" + d
	} else { 
		h = year + "-" + a + "-" + d + " " + k + ":" + p
	}
    
    b.fillText(h, g, l.getMiddle() + 1)
};
...
RangeSelectionPlotter.prototype.Draw = function (b) {
    ...
    b.fillStyle = d.getColor(Theme.Color.Text5);
    b.fill();
    b.strokeStyle = d.getColor(Theme.Color.Text5);
    b.stroke();
    b.font = d.getFont(Theme.Font.Default);
    b.textAlign = "left";
    b.textBaseline = "middle";
    ...
    if (f.getNameObject().getCompAt(2) == "main") {
        ...
        str = String.fromFloat(i, c);
    } else {
    	str = String.fromFloat1(i, 2);
    }
    b.fillText(str, a.getLeft() + 11 * kScale, g + 2 * kScale);
};
var ChartSettings = {};
ChartSettings.checkVersion = function () {
    if (ChartSettings._data.ver < 2) {
        ...
        a.period_weight = {};
        a.period_weight.line = 4;
        a.period_weight["1min"] = 9;
        a.period_weight["5min"] = 5;
        a.period_weight["15min"] = 10;
        a.period_weight["30min"] = 6;
        a.period_weight["1hour"] = 7;
        a.period_weight["1day"] = 8;
        a.period_weight["1week"] = 11;
        a.period_weight["1month"] = 12;
        a.period_weight["3min"] = 0;
        a.period_weight["2hour"] = 1;
        a.period_weight["4hour"] = 2;
        a.period_weight["6hour"] = 3;
...
function KLineMouseEvent() {
    ...
    $("#close_settings").click(function () {
        $("#chart_settings").removeClass("clicked")
    });
    $("#close_parameter_settings").click(function () {
    ...
    $("#chart_toolpanel .chart_toolpanel_button").click(function () {
        $("#chart_toolpanel .chart_toolpanel_button").removeClass("selectsed");
        if ($(this).children().attr("id") == "clearCanvas") {
        	$($("#chart_CrossCursor").parent()).addClass("selectsed");
        } else {
    ...
    $("#chart_tabbar li").click(function () {
        $("#chart_tabbar li").removeClass("active");
        $("#chart_select_main_indicator li").removeClass("active");
        $(this).addClass("active");
        
        var b =$($($(this)).children("a")[0]).attr("name");
        $("#chart_parameter_settings .chart_parameter_values .chart_parameter_value").css("display", "none");
        $("#chart_parameter_settings .chart_parameter_values ." + b).css("display", "block");
        
        var h = ChartManager.getInstance();
        var g = h.getIndicatorParameters(b);
        var c = 0;
        var f = [];
        $("#chart_parameter_settings .chart_parameter_values ." + b + " input").each(function () {
        	if ($(this).attr("name") == b) {
        ...
        $(this).find("a").each(function () {
        	if ($(this).hasClass("selectsed")) {
           		 $("#chart_tabbar li a").removeClass("selectsed");
    ...
    $("#chart_select_chart_style li").click(function () {
        $("#chart_select_chart_style a").removeClass("selectsed");
        $("#chart_select_chart_style li").removeClass("active");
        $(this).addClass("active");
        
        $(this).find("a").each(function () {
        	$(this).addClass("selectsed");
            var b = ChartSettings.get();
            b.charts.chartStyle = $(this).attr("name");
            ChartSettings.save();
            var c = ChartManager.getInstance();
            c.setChartStyle("frame0.k0", $(this).attr("name"));
            c.redraw();
            return;
        });
    });
    $("#chart_dropdown_themes li").click(function () {
        $("#chart_dropdown_themes li a").removeClass("selectsed");
        var b = $(this).attr("name");
    ...
    $("#chart_select_main_indicator li").click(function () {
        $("#chart_select_main_indicator li").removeClass("active");
        $("#chart_tabbar li").removeClass("active");
        $(this).addClass("active");
        var b =$($($(this)).children("a")[0]).attr("name");
        ...
        $(this).find("a").each(function () {
        	var hasSelectsed = false;
        	if ($(this).hasClass("selectsed")) {
        		hasSelectsed = true;
        	}
            $("#chart_select_main_indicator a").removeClass("selectsed");
            var b = $(this).attr("name");
            var c = ChartSettings.get();
            var d = ChartManager.getInstance();
            if (hasSelectsed) {
    ...
    $("#chart_select_theme li a").click(function () {
        $("#chart_select_theme a").removeClass("selectsed");
        if ($(this).attr("name") == "dark") {
            switch_theme("dark")
        } else {
            if ($(this).attr("name") == "light") {
    ...
    $("#chart_enable_tools li").click(function () {
    	 $(this).find("a").each(function () {
    ...
    $("#chart_toolpanel .show_btn_arrow").click(function() {
    	var s = $("#chart_enable_tools li .selectsed");
    	if (s.attr("name") == "on") {
            switch_tools("off")
        } else {
            if (s.attr("name") == "off") {
    ...
    $("#chart_enable_indicator li a").click(function () {
        $("#chart_enable_indicator a").removeClass("selectsed");
        if ($(this).attr("name") == "on") {
            switch_indic("on")
        } else {
            if ($(this).attr("name") == "off") {
    ...
    $("#chart_language_setting_div li a").click(function () {
        $("#chart_language_setting_div a").removeClass("selectsed");
    ...
    $("#chart_overlayCanvas").mousemove(function (f) {
        ...
        var b = (f.clientX - c.left) * kScale;
        var g = (f.clientY - c.top) * kScale;
        var d = ChartManager.getInstance();
        ...
        var b = (d.clientX - c.left) * kScale;
        var f = (d.clientY - c.top) * kScale;
        ChartManager.getInstance().onMouseDown("frame0", b, f)
    });
    $("#chart_parameter_settings :input").keyup(function () {
        var d = $(this).attr("name");
        var f = [];
        var h = ChartManager.getInstance();
        var g = h.getIndicatorParameters(d);
        var c = 0;
        $("#chart_parameter_settings ." + d + " :input").each(function () {
            if ($(this).attr("name") == d) {
                if ($(this).val() != "" && $(this).val() != null && $(this).val() != undefined) {
                    var j = parseInt($(this).val());
                    if (d != "MA" && d != "EMA") {
                        if (j < g[c].getMinValue()) {
                        	j = g[c].getMinValue();
                        } else if (j > g[c].getMaxValue()) {
                        	j = g[c].getMaxValue();
                        }
//                        $(this).val(j);
                    }
    ...
    $("#chart_parameter_settings button").click(function () {
    	var a = $("#chart_select_main_indicator li.active");
    	if (a.length == 0) {
    		a = $("#chart_tabbar li.active");
    	}
    	...
    	a.find("a").each(function () {
    		var c = $(this).attr("name");
             ...
             var e = [];
             $(".chart_parameter_values ." + c + " input").each(function () {
...
function RequestData(showLoading, types, name) {
	window.clearInterval(GLOBAL_VAR.taktime);
	GLOBAL_VAR.taktime = null;
	doRequestData(types, 0, name);
}
function stopOriRequest() {
	if (GLOBAL_VAR.taktime) {
		window.clearInterval(GLOBAL_VAR.taktime);
		GLOBAL_VAR.taktime = null;
	}
}
function dealKlineData(data, name) {
	var arrCu = [];
   	var currArrD = data;
   	var gapStr = TradingViewPeriodMap[name];
   	if (name == 'line') {
   		gapStr = '1';
   	}
   	var gap = 0;
   	if (gapStr == '1D' || gapStr == '3D' || gapStr == '1W' || gapStr == '1M') {
   		var more = 1;
   		if (gapStr == '1W') more = 7;
   		if (gapStr == '3D') more = 3;
   		gap = 3600 * 24 * more;
   	} else {
   		gap = parseInt(gapStr) * 60;
   	}
	for (var i = 0; i < currArrD.length; i++) {
		var date = (currArrD[i].time - gap) * 1000;
		if (gapStr == '1M') {
			var d = new Date(currArrD[i].time * 1000);
			d.setDate(1);
			if (d.getMonth() == 0) {
		...
		arrCu.push({
            date: date,
            open: currArrD[i].first,
            high: currArrD[i].high,
            low: currArrD[i].low,
            close: currArrD[i].last,
            volume: currArrD[i].vol,
            interval: gapStr
        });
	}
	requestDataUpdate(arrCu);
}
function sendRequestData(url, name, interval) {
	var nowRequest = GLOBAL_VAR.G_HTTP_REQUEST = $.ajax({
	       url: url,
	       type: "get",
	       dataType: "json",
	       ...
	       success: function (result) {
	           if (GLOBAL_VAR.G_HTTP_REQUEST == nowRequest) {
	               if (!result) {
	                   return
	               }
	               if (result != null) {
	            	   dealKlineData(result.data, name);
	               }
	               
	               stopOriRequest();
	               GLOBAL_VAR.taktime = setInterval("doRequestData('" + interval + "', 1,'" + name + "')", 2000);
	           }
	       },
...
function doRequestData(interval, last, name) {
	AbortRequest();
	var tradeType = "contract";
	if ($("#tradePath").val() == "vtrade") {
		...
		tradeType = $("#tradePath").val();
	}

	var symbol = $('#transaction_code').val();
	var url = window.location.origin + "/futuresApi/marketKline.do?interval=" + interval + "&type=" + tradeType + "&symbol=" + symbol + "&last=" + last;

	var newSubStr = "kline." + tradeType + '.' + symbol + "." + interval;
	if (klineSubStr != newSubStr || last == 0) {
		if (klineSubStr != newSubStr) {
			unsubKline();
			subKline(newSubStr);
		}
		
		sendRequestData(url, name, interval);
   } else {
...
function readCookie() {
    ...
    switch_period(d);
    $("#chart_period_" + d + "_v a").addClass("selectsed");
    $("#chart_period_" + d + "_h a").addClass("selectsed");
    ...
    a.find("a").each(function () {
        if ($(this).attr("name") == b.charts.mIndic) {
            ...
            $("#chart_parameter_settings .chart_parameter_values ." + b.charts.mIndic).css("display", "block");
            
            var h = ChartManager.getInstance();
            var g = h.getIndicatorParameters(b.charts.mIndic);
            var c = 0;
            $("#chart_parameter_settings .chart_parameter_values ." + b.charts.mIndic + " input").each(function () {
            	if ($(this).attr("name") == b.charts.mIndic) {
            		$(this).val(g[c].getValue());
            	}
            	c++;
    ...
    c.find("a").each(function () {
        if ($($(this)[0]).attr("name") == b.charts.chartStyle) {
            $(this).addClass("selectsed");
            $($(this).parent()).addClass("active");
        }
    });
...
var main = function () {
    ...
    $("#chart_toolbar_tabgroup li a").each(function() {
		if ($(this).hasClass("selectsed")) {
			this.click();
		}
	})
...
function refreshTemplate() {
    GLOBAL_VAR.chartMgr = DefaultTemplate.loadTemplate("frame0.k0", "BTC-Q");
    ChartManager.getInstance().redraw("All", true)
}
...
function chart_switch_language(b) {
    ...
    $("#chart_language_setting_div li a[name='" + b + "']").addClass("selectsed");
...
function on_size() {
    ...
    */
    ...
    $("#chart_mainCanvas").attr("width", $("#chart_canvasDraw").width() * kScale);
	$("#chart_mainCanvas").attr("height", $("#chart_canvasDraw").height() * kScale);
	$("#chart_overlayCanvas").attr("width", $("#chart_canvasDraw").width() * kScale);
	$("#chart_overlayCanvas").attr("height", $("#chart_canvasDraw").height() * kScale);
...
function switch_theme(c) {
    $("#chart_toolbar_theme a").removeClass("selectsed");
    $("#chart_select_theme a").removeClass("selectsed");
...
function switch_tools(a) {
    $("#chart_toolpanel .chart_toolpanel_button").removeClass("selectsed");
    $("#chart_enable_tools a").removeClass("selectsed");
    $("#chart_enable_tools li").removeClass("active");
    ...
    if (a == "on") {
        ...
        $("#chart_canvasGroup").css("padding-left", "60px");
        $("#chart_toolpanel").css("width", "60px");
        $("#chart_toolpanel")[0].style.display = "flex";
        $("#chart_toolpanel .chart_toolpanel_items").css("display", "block");
        $("#chart_toolpanel .show_btn_arrow svg").css("transform", "none");
        ...
        if (a == "off") {
            ...
            $("#chart_canvasGroup").css("padding-left", "8px");
            $("#chart_toolpanel").css("width", "8px");
            $("#chart_toolpanel .chart_toolpanel_items").css("display", "none");
            $("#chart_toolpanel .show_btn_arrow svg").css("transform", "rotate(180deg)");
...
function switch_indic(a) {
    $("#chart_enable_indicator a").removeClass("selectsed");
    $("#chart_enable_indicator a[name='" + a + "']").addClass("selectsed");
    if (a == "on") {
        var b = ChartSettings.get();
        ...
        if (a == "off") {
            ...
            $("#chart_tabbar a").removeClass("selectsed")
        }
    }
    on_size()
}
...
function switch_market_selected(b) {
    $(".market_chooser ul a").removeClass("selectsed");
    $(".market_chooser ul a[name='" + b + "']").addClass("selectsed");
