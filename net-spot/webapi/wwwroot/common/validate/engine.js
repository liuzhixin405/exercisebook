(function($) {
	...
	var methods = {
		...
		_onSubmitEvent : function() {
			...
			if (r && options.oneSubmitNum) {
				form.unbind('submit');
				//取消再次点击的提交时间
				$(form).find('button[type=submit]').html('处理中，请稍后');
		...
		_validateField : function(field, options, skipAjaxValidation) {
			...
			for ( var i = 0, field_errors = 0; i < rules.length; i++) {
				...
				case "maxBuy":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._maxBuy);
					break;
				case "past":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
		...
		_maxBuy : function(field, rules, i, options) {
			var max = parseFloat(rules[i + 1]);
			var len = parseFloat(field.val());
			if (len > max) {
				var rule = options.allrules.maxBuy;
				if (rule.alertText2)
					return rule.alertText + max + rule.alertText2;
				//orefalo: to review, also do the translations
				return rule.alertText + max;
		...
		_isDate : function(value) {
			...
			return dateRegEx.test($.trim(value));
		},
		/**
		...
		_isDateTime : function(value) {
			...
			return dateTimeRegEx.test($.trim(value));
		},
		...
		_ajax : function(field, rules, i, options) {
			...
			case 'mairuliang':
				fieldname = 'volume';
				data[fieldname] = fieldval;
				data['price'] = $('#mairujia').val();
				break;
			case 'mairuliang2':
				fieldname = 'volume';
				data[fieldname] = fieldval;
				data['price'] = $('#mairujia2').val();
				break;
			case 'mairuliang_m':
				fieldname = 'volume';
				data[fieldname] = fieldval;
				data['price'] = $('#mairujia_m').val();
				break;
			case 'volume':
				fieldname = 'volume';
				data[fieldname] = fieldval;
			...
			case 'amt':
				...
				data[fieldname] = fieldval;
				data['my_channel_id'] = $(
						'input[@type=radio][name=my_channel_id][checked]')
						.val();
			default:
				data[fieldname] = fieldval;
				break;
	...
	$(function() {
		$.validationEngine.defaults.promptPosition = methods.isRTL() ? 'topRight'
				: "topLeft"
	});
})(jQuery);