...
namespace Orleans.Host
{
    ...
    .AddJsonFile("orleansconfig.json") // 添加 Json 文件配置
    .Build();
            var siloOptions = jsonConfiguration.GetSection("host").Get<SiloExOptions>();
            var builder = new SiloHostBuilder()
                 //.UseLocalhostClustering(siloOptions.SiloPort,siloOptions.GatewayPort)
                 .UseDevelopmentClustering(new IPEndPoint(IPAddress.Parse(siloOptions.IpAddress), siloOptions.SiloPort))
