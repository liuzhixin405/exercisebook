...
namespace CqrsWithEs.Domain.Policy
{
    public class Policy : AggregateRoot
    {
     ...
     => new Policy(Guid.NewGuid(), offer, purchaseDate, policyStartDate);
        private Policy(Guid policyId,Offer.Offer offer,DateTime purchaseDate,DateTime policyStartDate)
        {
            ...
            var coverPeriod = ValidityPeriod.Between(policyStartDate, policyStartDate.Add(offer.CoverPeriod));
            var covers = offer.Covers.Select(c => PolicyCover.ForPrice(c, coverPeriod)).ToList();
            Id = policyId;
        ...
        private bool Terminated()=>
            versions.Any(v=>v.VersionStatus== PolicyVersionStatus.Active && v.PolicyStatus== PolicyStatus.Terminated);
        #endregion
    }
}
