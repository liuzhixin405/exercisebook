...
namespace eapi.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        {
            await orderService.GetGrain<IOrderGrains>(0).CreateTestLock(sku, count); //执行时间快,库存少量扣减 10s
        }
        [HttpPost]
        public async Task CreateBlockingLock(string sku, int count)//阻塞锁
        {
            await orderService.GetGrain<IOrderGrains>(0).CreateBlockingLock(sku, count); //卖不完,时间长 50s
        }
        [HttpPost]
        public async Task CreateDistLock(string sku, int count) //colder组件 分布式锁
        {
            await orderService.GetGrain<IOrderGrains>(0).CreateDistLock(sku, count); //库存扣完，时间长 50s
        }

        [HttpPost]
        public async Task CreateNetLock(string sku, int count)   //netlock.net锁 
        {
            await orderService.GetGrain<IOrderGrains>(0).CreateNetLock(sku, count); //库存扣完，时间长 50s
        }
        ...
        {
             orderService.GetGrain<IOrderGrains>(0).CreateLocalLock(sku, count); //
        }

        [HttpPost]
        public async Task CreateNoLock(string sku, int count)
        {
           await orderService.GetGrain<IOrderGrains>(0).CreateNoLock(sku, count); //乱的
        }
        [HttpGet]
